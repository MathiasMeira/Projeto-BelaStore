...
<?php
	...
	$comando = "SELECT * FROM produto WHERE idcategoria = $idcategoria";
	$result = mysqli_query($conexao, $comando);
	$linha = mysqli_fetch_assoc($result);

	$comando2 = "SELECT * FROM categoria WHERE idcategoria = $idcategoria";
	$resultado = mysqli_query($conexao, $comando2);
	$linha2 = mysqli_fetch_assoc($resultado);
?>
<div id="tabela2">
	<form action="alterar_produto.php" method="POST">
		<input type="hidden" name="hdidcategoria" value="<?=$idcategoria?>">
		Nome: <input type="text" name="nome" value="<?=$linha['nomeproduto']?>"><br>
		Preço: <input type="text" name="preco" value="<?=$linha['preco']?>"><br>
		Estoque Mínimo: <input type="text" name="estoque_minimo" value="<?=$linha['estoque_minimo']?>"><br>
		Estoque Máximo: <input type="text" name="estoque_maximo" value="<?=$linha['estoque_maximo']?>"><br>
